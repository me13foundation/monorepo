...
1. Creates default MED13 research space
2. Migrates existing data sources to MED13 space
...
def get_or_create_med13_space(
    session: Session,
    space_repo: SqlAlchemyResearchSpaceRepository,
) -> tuple[ResearchSpace, UUID]:
    """
    Get existing MED13 space or create a new one.
    ...
    Returns:
        The MED13 research space entity
    """
    # Check if MED13 space already exists
    existing_space = space_repo.find_by_slug("med13")
    if existing_space:
        logger.info("Found existing MED13 space: %s", existing_space.id)
        med13_space_id = existing_space.id
        MIGRATION_METADATA["med13_space_id"] = str(med13_space_id)
        return existing_space, med13_space_id
    ...
    if not admin_user:
        msg = "No users found in database. Cannot create MED13 space."
        raise ValueError(msg)

    owner_id = UUID(admin_user.id) if isinstance(admin_user.id, str) else admin_user.id

    # Create MED13 research space
    logger.info("Creating MED13 research space with owner: %s", owner_id)
    med13_space = ResearchSpace(
        id=uuid4(),  # Generate UUID for the space
        slug="med13",
        name="MED13 Research Space",
        description="Default research space for MED13 syndrome research and curation",
        owner_id=owner_id,
        status=SpaceStatus.ACTIVE,
        settings={"is_default": True, "migrated_from_legacy": True},
        tags=["med13", "syndrome", "default"],
    )

    created_space = space_repo.save(med13_space)
    logger.info("Created MED13 space: %s", created_space.id)
    MIGRATION_METADATA["med13_space_id"] = str(created_space.id)
...
def migrate_data_sources_to_space(
    ...
    *,
    ...
    """
    Migrate all existing data sources to MED13 research space.

    Args:
        session: Database session
        med13_space_id: The MED13 research space ID
    ...
    # Update all orphaned sources to belong to MED13 space
    updated_count = session.execute(
        update(UserDataSourceModel)
        .where(UserDataSourceModel.research_space_id.is_(None))
        .values(research_space_id=str(med13_space_id)),
    ).rowcount

    session.commit()
    logger.info("Migrated %d data sources to MED13 space", updated_count)
...
def create_memberships_for_users(
    ...
    *,
    ...
    """
    Create memberships for all existing users in MED13 space.
    ...
    for user_model in all_users:
        ...
        existing = membership_repo.find_by_space_and_user(med13_space_id, user_id)
        if existing:
...
def verify_data_integrity(
    ...
    # Check that MED13 space exists
    space_stmt = select(ResearchSpaceModel).where(
        ResearchSpaceModel.id == str(med13_space_id),
    )
    space_exists = session.execute(space_stmt).scalar_one_or_none() is not None
    results["med13_space_exists"] = space_exists
    logger.info("MED13 space exists: %s", space_exists)
    ...
    membership_stmt = select(ResearchSpaceMembershipModel).where(
        ResearchSpaceMembershipModel.space_id == str(med13_space_id),
    ...
    logger.info("Active memberships in MED13 space: %d", membership_count)
...
def rollback_migration(session: Session) -> None:
    ...
    # Note: We don't delete the MED13 space or memberships in rollback
...
def main() -> int:  # noqa: PLR0915
    ...
    try:
        ...
        # Step 1: Create or get MED13 research space
        logger.info("Step 1: Creating/getting MED13 research space...")
        _med13_space, med13_space_id = get_or_create_med13_space(session, space_repo)
        logger.info("MED13 space ID: %s", med13_space_id)
        logger.info("")

        # Step 2: Migrate data sources
        logger.info("Step 2: Migrating data sources to MED13 space...")
        ...
        if not args.dry_run:
            ...
            verification_results = verify_data_integrity(session, med13_space_id)
            logger.info("")
