...
export default function TemplateDetailPage({ params }: TemplateDetailPageProps) {
  ...
  const { data, isLoading, error } = useTemplate(params.templateId)
  ...
  const invalidateTemplateQueries = async () => {
    ...
    await queryClient.invalidateQueries({ queryKey: ['template', params.templateId] })
  }
  ...
  const handleUpdate = async (payload: TemplateUpdatePayload['data']) => {
    await updateTemplate({ templateId: params.templateId, data: payload })
    await invalidateTemplateQueries()
    setEditOpen(false)
  }
  ...
  const handleValidationRulesSave = async (rules: TemplateValidationRule[]) => {
    setRulesSaving(true)
    try {
      await updateTemplate({ templateId: params.templateId, data: { validation_rules: rules } })
      await invalidateTemplateQueries()
    } finally {
  ...
  const handleApprove = async () => {
    ...
    try {
      await approveTemplate(params.templateId)
      await invalidateTemplateQueries()
    } finally {
  ...
  const handlePublish = async () => {
    ...
    try {
      await publishTemplate(params.templateId)
      await invalidateTemplateQueries()
    } finally {
