"""SQLAlchemy implementation for inspecting Flujo state tables."""
...
class SqlAlchemyFlujoStateRepository(FlujoStatePort):
    """Read-only Flujo state inspector backed by SQLAlchemy."""
    ...
    def find_latest_run_id(self, *, since: datetime) -> str | None:
        try:
            ...
            logger.warning(
                "Flujo schema not available for run lookup; skipping Flujo metadata. %s",
                exc,
            )
            return None
    ...
    def _fetch_sample_rows(self, table: str, run_id: str) -> list[JSONObject]:
        ...
        try:
            ...
            logger.warning(
                "Failed to fetch Flujo sample rows for %s; returning empty. %s",
                table,
                exc,
            )
            return []
...
__all__ = ["SqlAlchemyFlujoStateRepository"]
