...
def upgrade() -> None:
    ...
    if not inspector.has_table("research_spaces"):
        op.create_table(
            "research_spaces",
    ...
    if not inspector.has_table("research_space_memberships"):
        op.create_table(
            ...
            sa.PrimaryKeyConstraint(
                "id",
                name=op.f("pk_research_space_memberships"),
            ),
    ...
    if is_sqlite:
        existing_columns = {
            column["name"] for column in inspector.get_columns("user_data_sources")
        }
        if "research_space_id" not in existing_columns:
            op.add_column(
                "user_data_sources",
                sa.Column("research_space_id", sa.String(length=36), nullable=True),
            )
            op.create_index(
